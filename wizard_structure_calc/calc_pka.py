...
index = int(2 * (duplicate[0] * duplicate[1] * duplicate[2] / 2 + duplicate[0] * duplicate[1] / 2 + duplicate[0] / 2)) - 1
...
initial = bulk('W', 'bcc', lc, cubic = True) * duplicate
initial.info['group'] = group
Morph(initial).gpumd('test/relax', 
...
atoms = read_xyz('relax/restart.xyz')[0]
Morph(atoms).set_pka(pka_energy, angle, index)

Morph(atoms).gpumd('cascade',
